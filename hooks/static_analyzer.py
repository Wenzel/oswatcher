...
class StaticAnalyzerHook(Hook):
    ...
    def has_catSignature(self):
        ...
        #2.2 compare each OCTET STRING with the PE image hash (ContentInfo->Digest using lief)
        #2.3 if there is a match, the file is catalog-signed, return true, else return false
        pass
    ...
    def handle_new_file(self, event):
        ...
        if mime_type in self.VALID_MIME_APP:	
            ...
            dynamicBase = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.DYNAMIC_BASE)
            noSEH = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.NO_SEH)
            guardCF = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.GUARD_CF)
            forceIntegrity = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.FORCE_INTEGRITY)
            nxCompat = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.NX_COMPAT)
            highEntropyVA = pe.optional_header.has(lief.PE.DLL_CHARACTERISTICS.HIGH_ENTROPY_VA)
            hasEmbeddedSig = pe.has_signature
            hasCatSig = self.has_catSignature() #finish function: execute only if hasEmbeddedSig is false 
            ...
            check_pe = checkPE(dynamicBase, noSEH, guardCF, forceIntegrity, nxCompat, highEntropyVA, codeSize, numFunctionsExported, imageSize, hasEmbeddedSig, hasCatSig, importedLibs)
            print(check_pe)
            #self.context.trigger('security_checksec_pe', inode=inode, check_pe=check_pe)
